...
CMediaDemux::CMediaDemux(const char *fileName)
  :m_pFormatCtx(NULL),m_pvideoCodec(NULL),m_pvideoCodecContext(NULL),m_paudioCodec(NULL), m_paudioCodecContext(NULL),m_audioCodeId(AV_CODEC_ID_NONE),m_videoCodeId(AV_CODEC_ID_NONE),m_pSrcFile(fileName),m_notSetUp(true)
{
  //nothing to do;
  setUp();
}
